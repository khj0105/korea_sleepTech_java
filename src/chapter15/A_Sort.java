...
public class A_Sort {
	public static void main(String[] args) {
		...
		// 2. 리스트 정렬
		// 1) 기본 정렬: sort() - 오름차순, 
		//				, Collections.sort(데이터, Collections.revers.Order()); - 내림차순
		
		// 2) Comparator 클래스 사용
		List<String> names = new ArrayList<String>(List.of("Tom", "Jerry", "Pororo", "Loppy"));
...
//		names.sort(Comparator.comparingInt(name -> name.length()));
		...
		// 3) 스트림 API 사용 정렬
		// : 정렬된 결과를 새로운 리스트로 반환
		List<Integer> listNumbers = new ArrayList<Integer>(List.of(8, 2, 4, 5, 1));
		...
		List<String> characterNames = new ArrayList<String>(List.of("Tom", "Jerry", "Pororo", "Loppy"));
		// 캐릭터 이름의 길이를 오름차순 정렬
		...
		System.out.println(sortedCharacter); // [Pororo, Jerry, Loppy, Tom]
