...
import lombok.*; // 롬북 라이브러리 전체 클래스를 import

// == Lombok(롬복) 라이브러리 == //
// : 보일러플레이트 코드를 줄여주는 라이브러리
// - 어노테이션을 사용해서 코드를 간결하게 만들어 주는 역할
...
class LomokClass {
	...
	//		: 필수값을 매개변수로 받는 생성자
	//		- final 필드값 또는 @NonNull 이노테이션을 매개변수로 받음
	...
	// - setter는 필드의 타입과 필드명을 가지는 매개변수를 가지는 메서드
...
//		LomokClass lombokClass = new LomokClass();
//		lombokClass.setTitle("백설공주");
//		lombokClass.setAuthor("이도경");
//		System.out.println(lombokClass); // LomokClass(title=백설공주, author=이도경)
		
		//LomokClass lombokClass1 = new LomokClass("백설공주");
		//lombokClass1.setTitle("신데렐라");
		// : @Setter는 final 필드에 대해서 메서드 생성 X
		
		LomokClass lombokClass2 = new LomokClass("신데렐라", "이승아");
		
		//System.out.println(lombokClass1); // LomokClass(title=백설공주, author=null)
		System.out.println(lombokClass2); // LomokClass(title=백설공주, author=null)
		
		LomokClass lombokClass3 = LomokClass.builder()
				.author("루피")
				.build();
